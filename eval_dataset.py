...
def main(log_dir=None):
    ...
    # get_scores(train_id_path, 'train', log_dir)
    # test_id_path = f'./annotation/{cfg.data.target}/{split_id}/test_id.csv'
    # get_scores(test_id_path, 'test', log_dir)
    id_path = f'./annotation/elise/datalist.csv'
    get_scores(id_path, 'elise_all', log_dir)


def get_scores(id_path, target, log_dir):
    scores = {}
