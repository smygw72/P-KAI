...
def _PDR_loss(cfg, sup_outs_dif, sup_outs_sim, inf_outs_dif, inf_outs_sim):
    dif_loss = _get_dif_loss(cfg, sup_outs_dif[0], inf_outs_dif[0])
    sim_loss = _get_sim_loss(cfg, sup_outs_sim[0], inf_outs_sim[0])
    return dif_loss, sim_loss


def _APR_loss(cfg, sup_outs_dif, sup_outs_sim, inf_outs_dif, inf_outs_sim):

    dif_loss_rx_good = _get_dif_loss(cfg, sup_outs_dif[0], inf_outs_dif[0])
    dif_loss_ax_good = _get_dif_loss(cfg, sup_outs_dif[1], inf_outs_dif[1])
    sim_loss_rx_good = _get_sim_loss(cfg, sup_outs_sim[0], inf_outs_sim[0])
    sim_loss_ax_good = _get_sim_loss(cfg, sup_outs_sim[1], inf_outs_sim[1])
...
def _get_acc(cfg, sup_outs_dif, sup_outs_sim, inf_outs_dif, inf_outs_sim):
    arch = cfg.model.architecture

    if (arch == 'PDR') or (cfg.model.disable_bad is True):
...
def _get_dif_loss(cfg, dif1, dif2):
    ...
    elif cfg.learning.loss.method == 'softplus':
...
def get_metrics(cfg, sup_outs, inf_outs, label_sim):
    ...
    # accuracy
    dif_acc, sim_acc = _get_acc(cfg, sup_outs_dif, sup_outs_sim, inf_outs_dif, inf_outs_sim)
    # TODO: sim_accを実装
    total_acc = dif_acc
